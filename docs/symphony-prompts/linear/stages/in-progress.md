...
1. **Conflict pre-check.** List other active issues (state `In Progress`,
   `Review`, or `QA`) and read their `docs/<other-id>/explore/` Touched
   Files for overlap with this ticket's `## Touched Files`. If overlap
   exists, transition state to `Blocked` and post a `## Conflict` comment
   listing the other issue id, the overlapping path(s), and a one-line
   reason ("waiting on <ID> to finish editing <path>"); STOP and do not
   implement.
2. **Read shared context first.** Open `docs/{{ issue.identifier }}/explore/`
   ...
   are the contract.
3. Implement the chosen option from the Explore Recommendation (or, on a
...
6. Open a draft PR. Post an Implementation comment with the PR link, the
