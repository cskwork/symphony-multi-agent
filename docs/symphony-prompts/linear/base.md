...
it must come *after* a plain-language header.
...
- **Rewind cap.** Count Review Findings + QA Failure comments on this
  issue. If the count is already at or above `agent.max_attempts` in
  `WORKFLOW.md` (default 5) and you would otherwise rewind to `In Progress`,
  transition to `Blocked` instead and post a `## Budget Exceeded` comment
  naming the loop (which stages, how many rounds, the last unresolved item).
