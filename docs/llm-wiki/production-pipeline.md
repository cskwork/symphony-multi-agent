...
- `tracker.active_states` in both example WORKFLOW files must equal
  `[Todo, Explore, Plan, "In Progress", Review, QA, Learn]` in that order.
...
- `tests/test_workflow_pipeline_prompt.py` parametrizes both example
  WORKFLOW files and asserts: (a) every state in the eight-stage list
  appears in `active_states`, (b) every stage heading (`TRIAGE`,
  `EXPLORE`, `PLAN`, `IMPLEMENT`, `REVIEW`, `QA`, `LEARN`, `DONE`) renders for
...
- The diagram in `WORKFLOW.file.example.md` (and its mirror in
  ...
  rules under each `### STAGE` heading as authoritative.
...
**Files of interest:**
- `WORKFLOW.file.example.md` -- `active_states`, file-board hooks, and
  prompt stage map for the file tracker.
- `WORKFLOW.example.md` -- same pipeline, Linear variant.
- `docs/symphony-prompts/{file,linear}/stages/` -- per-stage rules
  (TRIAGE / EXPLORE / PLAN / IMPLEMENT / REVIEW / QA / LEARN / DONE).
...
- `tests/test_workflow_pipeline_prompt.py` -- the literal phrase lists
  and render tests that any pipeline-prompt change must keep satisfying.

**Decision log:**
- 2026-05-16 | Plan stage | Added Plan between Explore and In Progress.
